...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("RT-SLAM: Map Viewer",1024,768);
    ...
    Ow.SetIdentity();
    cv::namedWindow("RT-SLAM: Current Frame");
    ...
    while(1)
    {
        ...
        // Rt
        {
            ...
            for (auto vit = mmDetectMap.begin(); vit != mmDetectMap.end(); vit++)
            {
                if (vit->second.size() != 0)
                {
                    ...
                    {
                        cv::rectangle(toShow, area, cv::Scalar(255, 0, 255), 2);
                        cv::putText(toShow,
                                    vit->first,
        ...
        cv::imshow("RT-SLAM: Current Frame",toShow);
        cv::waitKey(mT);
